...
hcp_registry_exists() {
    ...
    debug "${FUNCNAME}: Finished execution"
    if [[ ${RESPONSE} == 200 ]]; then
        debug "The HCP Packer registry already exist"
        return 0
...
create_hcp_packer_registry() {
    ...
    hcp_registry_exists && {
        warning "An HCP Packer registry already exist."
        return
    }

    RESPONSE_BODY=$(mktemp)

    debug "${FUNCNAME}: Creating HCP Packer registry."
    RESPONSE=$(
        curl -s --location "${BASE_HCP_PACKER_URL}/${l_hcp_organization_id}/projects/${l_hcp_project_id}/registry" \
            --header "Authorization: Bearer ${HCP_TOKEN}" \
            --header "Content-Type: application/json" \
    ...
    if [ ${RESPONSE} != "200" ]; then
        error "Failed to create HCP Packer Registry (response code: ${RESPONSE})."
        error "$(cat ${RESPONSE_BODY})"
        return
    fi
    cat ${RESPONSE_BODY}
    ...
    RESPONSE=$(
        curl -v --location "${BASE_HCP_OPERATIONS_URL}/${l_hcp_organization_id}/projects/${l_hcp_project_id}/operations/${OPERATION_ID}/wait" \
            --header "Authorization: Bearer ${HCP_TOKEN}" \
            --header "Content-Type: application/json" \
            --output "${RESPONSE_BODY}" \
            --write-out "%{http_code}" \
    ...
    if [ ${RESPONSE} != "200" ]; then
        error "Failed to wait for creation of HCP Packer Registry (response code: ${RESPONSE})."
        error "$(cat ${RESPONSE_BODY})"
        return
    fi
    cat ${RESPONSE_BODY}
